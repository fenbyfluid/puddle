...
impl ScreenSpec {
    fn encode(&self) -> Result<Vec<u8>> {
        ...
        for (area, animation_type, animation_duration, animation_delay) in [
            (&self.left_main, &self.left_animation_type, &self.left_animation_duration, &self.left_animation_delay),
            (&self.right_main, &self.right_animation_type, &self.right_animation_duration, &self.right_animation_delay),
        ] {
            match area {
            ...
            payload.push((*animation_type).into());
            payload.extend(animation_duration.to_le_bytes());
            payload.extend(animation_delay.to_le_bytes());
        }
...
mod tests {
    ...
    fn test_send_screen_update_fragmentation() {
        ...
        assert_eq!(
            reports[0],
            [
                0x01, 0x01, 0x03, 0x00, 0x4F, 0x6E, 0x65, 0x00, 0x54, 0x77, 0x6F, 0x00, 0x54, 0x68, 0x72, 0x65, 0x65,
        ...
        assert_eq!(
            reports[1],
            [
                0x01, 0x01, 0x03, 0x01, 0x20, 0x31, 0x00, 0x02, 0x00, 0x49, 0x74, 0x65, 0x6D, 0x20, 0x32, 0x00, 0x03,
                0x01, 0x49, 0x74, 0x65, 0x6D, 0x20, 0x33, 0x00, 0x04, 0x01, 0x49, 0x74, 0x65, 0x6D, 0x20, 0x34, 0x00,
                0x07, 0x2C, 0x01, 0x00, 0x00, 0x00, 0x00, 0x03, 0x4C, 0x69, 0x6E, 0x65, 0x20, 0x31, 0x00, 0x4C, 0x69,
                0x6E, 0x65, 0x20, 0x32, 0x00, 0x4C, 0x69, 0x6E, 0x65, 0x20, 0x33, 0x00, 0x07,
            ]
        );
        assert_eq!(
            reports[2],
            [
                0x01, 0x01, 0x03, 0x02, 0x2C, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
